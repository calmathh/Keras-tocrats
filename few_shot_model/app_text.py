...
if uploaded_file:
    ...
    resized_path = os.path.join("temp", "resized_" + uploaded_file.name)
    ...
    try:
        ...
        st.write(f"Original image size: {img.size}")

        max_size = (1024, 1024)  # Set maximum allowed dimensions (adjust as needed)
        if img.size[0] > max_size[0] or img.size[1] > max_size[1]:
            img.thumbnail(max_size, Image.LANCZOS)
            img.save(resized_path)
            st.write(f"Image resized to: {img.size}")
            input_path = resized_path  # Update input_path to use resized image
        else:
            st.write("Image is within acceptable size limits.")
    except Exception as e:
        st.error(f"Error processing the image: {e}")
        st.stop()

    # Display the uploaded/resized image
